...
def managerList():
    if sessions.check(request.cookies):
        try:
            ...
            # "SELECT acls.mac, acls.name, acls.cluster, user.username FROM user, acls LEFT JOIN share ON acls.mac=share.mac WHERE acls.user=(SELECT id FROM user WHERE username=%s) GROUP BY acls.mac"
            ...
            if main != None:
                for i in range(len(main)):
                    response['own'].append(
                        {'mac': main[i][0], 'name': main[i][1], 'cluster': main[i][2], 'shareWith': main[i][3]})
            if secondary != None:
                for i in range(len(secondary)):
                    if secondary[i][0] != None:
                        response['share'].append(
                            {'mac': secondary[i][0], 'name': secondary[i][1], 'shareBy': secondary[i][2]})
    ...
    if sessions.check(request.cookies):
        ...
        if type == "0":  # change name or cluster
            ...
            if re.match("^[A-Za-z0-9_-]*$", name) and len(name) < 16:
                q = ddbb.query(
                    "UPDATE acls SET name=%s, cluster=%s WHERE user=(SELECT id FROM user WHERE username=%s) AND mac=%s", name, cluster, user, mac)
                if q != None:
            ...
            email = base64.b64decode(
                request.headers.get("email")).decode('utf-8')
            ...
            if re.match("^[A-Za-z0-9_-]*$", name) and len(name) < 16:
                q = ddbb.query(
                    "INSERT INTO acls (mac, user, name) VALUES (%s, (SELECT id FROM user WHERE username=%s), %s)", mac, user, name)
                if q != None:
