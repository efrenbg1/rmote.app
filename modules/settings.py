...
def settingsDestroy():
    if sessions.check(request.cookies):
        try:
            ...
            pw = base64.b64decode(request.headers.get("pw")).decode('utf-8')
            if ddbb.checkPW(user, pw):
                    share = ddbb.query("DELETE FROM share WHERE owner=(SELECT id FROM user WHERE username=%s) OR user=(SELECT id FROM user WHERE username=%s)", user, user)
                    if share != None:
                        acls = ddbb.query("DELETE FROM acls WHERE user=(SELECT id FROM user WHERE username=%s)", user)
                        if acls != None:
                            ddbb.acls.delete(user)
                            account = ddbb.query("DELETE FROM user WHERE username=%s", user)
                            if account != None:
                                ddbb.users.delete(user)
                                ddbb.sessions.delete(user)
                                return str(json.dumps({'Done': '1'}))
