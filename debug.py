...
class changesListener(FileSystemEventHandler):
    ...
    def on_any_event(self, event):
        ...
        js = event.src_path.endswith(".js")
        if (py or js) and (time.time() - self.t) > 5 and not self.mutex.locked():
            self.mutex.acquire()
            print(colored("\nReloading app...\n\n", "red"))
            self.t = time.time()
            time.sleep(1)
...
server.watch('static/recover.html', delay=2)
server.watch('static/register.html', delay=2)
