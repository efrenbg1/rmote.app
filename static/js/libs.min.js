...
callback();},400);}window.addEventListener('popstate',function(event){if(event.state==undefined||event.state.session!==nav.session||document.getElementById('login').style.display=="block"){window.history.forward();return;}
...
return location;}
reconstruct(){if(this.order===0){if(this.section().length===0){this.setSection(['home']);}
if(this.section().length===1){this.setSection([this.section()[0]]);}else{var section=this.section();this.setSection([section[0]]);for(var i=2;i<=section.length;i=i+2){this.setSection(section.slice(0,i));}}}
if(this.changes(this.section())){return;}
...
load(id){var section=this.section();this.setSection([id]);try{if(this.changes(section)){tools.showModal('unsaved');window.history.back();return}}catch(e){}
tools.animationTime=performance.now();$('#grid')[0].setAttribute('class','slide-out');setTimeout(function(){this.reconstruct();}.bind(this),100);if(document.getElementById("content").style.display==="none"){tools.hideNavbar();}}
setSection(name,replace){this.order++;name=name.filter(entry=>entry!=="");var url=window.location.origin;this.old=[];var title="";var last="";var icon="";for(var i=0;i<name.length;i++){var steps=(name.length/2)-1-Math.floor(i/2);if(i==0)steps=Math.floor(name.length/2);this.old.push(name[i].toString());url+="/"+name[i];if(i==0){last=this.modules[name[i]].translation;icon=this.modules[name[i]].icon;icon=(icon!=""&&icon!=undefined)?`<i data-feather="{}"></i>&nbsp;`.format(icon):"";if(steps===0){title+=`<li class="breadcrumb-item active" aria-current="page"><p class="mb-0">{}{}</p></li>`.format(icon,last);}else{title+=`<li class="breadcrumb-item"><a href="javascript:window.history.go(-{})"><b>{}{}</b></a></li>`.format(steps,icon,last);}}else if(i==1){last+=" "+name[i];if(steps===0){title+=`<li class="breadcrumb-item active" aria-current="page">{}</li>`.format(name[i]);}else{title+=`<li class="breadcrumb-item"><a href="javascript:window.history.go(-{})">{}</a></li>`.format(steps,name[i]);}}else{last=this.modules[name[i]].translation+" "+name[i+1];icon=this.modules[name[i]].icon;icon=(icon!=""&&icon!=undefined)?`<i data-feather="{}"></i>&nbsp;`.format(icon):"";url+="/"+name[i+1];this.old.push(name[i+1].toString());if(steps===0){title+=`<li class="breadcrumb-item active" aria-current="page"><p class="mb-0">{}{}</p></li>`.format(icon,last);}else{title+=`<li class="breadcrumb-item"><a href="javascript:window.history.go(-{})">{}{}&nbsp;{}</a></li>`.format(steps,icon,last);}
i++;}}
this.title(title);if(replace){history.replaceState({prev:this.old,order:this.order,session:this.session,title:title,nav:prefix+" - "+last},"",url);}else{history.pushState({prev:this.old,order:this.order,session:this.session,title:title,nav:prefix+" - "+last},"",url);}
document.title=prefix+" - "+last;}
title(string){document.getElementById("titleDesktop").innerHTML=string;document.getElementById("titleMobile").innerHTML=string;try{feather.replace();}catch(e){console.log(e);}}}console.log("hey")
class Session{constructor(){this.templates=new sessionTemplates();}
...
logIn(){document.getElementById("obfuscator").classList.remove('is-visible');var user=document.getElementById("user").value;var pw=document.getElementById("pw").value;tools.hideDiag();tools.showLoading('profile');tools.req('/login',function(status,response){tools.hideDiag();if(status===200){console.log(response);tools.setCookie("Username",response['Username'],5);tools.setCookie("Session",response['Cookie'],5);document.getElementById("username").innerText=tools.getCookie("Username");section.reconstruct();}else{tools.showLogIn("Wrong credentials ("+status+")");}}.bind(this),{'Username':tools.encodeSTR(user),'Password':tools.encodeSTR(pw)});}
logOut(){clearInterval(tools.interval);tools.req('/logout',function(status,response){document.cookie="Username=;expires=Thu, 01 Jan 1970 00:00:01 GMT;";document.cookie="Session=;expires=Thu, 01 Jan 1970 00:00:01 GMT;";tools.showLogIn("Session closed");}.bind(this));}
...
this.opened=false;}
sreq(event,callback,headers){clearTimeout(this.uReqTimer);if(!session.check()){this.sClose();return;}
...
this.socket.emit(event,headers,function(response,status){if(typeof status!="number")status=200;callback(status,response);}.bind(this));}
...
showFailure(code){document.getElementById("toastContent").innerHTML=`<i data-feather="alert-triangle" class="rounded mr-1 inline-feather" color="red"></i>
            <strong class="mr-auto" style="color:red;">Error al ejecutar la acción&nbsp;({})</strong>`.format(code);$("#toast").toast({delay:6000});$("#toast").toast("show");feather.replace();}
...
String.prototype.format=function(){var i=0,args=arguments;return this.replace(/{}/g,function(){return typeof args[i]!='undefined'?args[i++]:'';});};String.prototype.render=function(args){var reg="";Object.keys(args).forEach(n=>{reg+=(reg!=""?"|":"")+"{{"+n+"}}";});var re=new RegExp(reg,"gi");return this.replace(re,function(match){var str=args[match.substring(2,match.length-2)];return str!=null?str:"";});};