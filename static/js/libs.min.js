...
showLogIn(msg){clearInterval(tools.interval);document.getElementById("obfuscator").classList.add('is-visible');showDialog({text:session.templates.login().format(msg),cancelable:false,onLoaded:function(){},});window.addEventListener('keydown',this.enter,false);}
...
<p></p>`;showDialog({text:html.format(msg),cancelable:true,onHidden:function(){if(!session.check()){tools.showLogIn('Session expired','');}},});}
