class Control{constructor(){this.templates=new controlTemplates();this.getCards();}
...
getCards(){tools.showLoading("data");tools.req('/control/list',function(status,response){if(status===200){console.log(response);var cards="";for(var i=0;i<response['own'].length;i++){var MAC=response['own'][i]['mac'];var name=response['own'][i]['name'];var update=this.templates.update;if(parseInt(response['own'][i]['version'])==3){update="";}
cards=cards+this.templates.card.format(MAC,MAC,MAC,MAC,update,name,MAC,MAC,MAC,MAC,MAC,MAC,MAC,MAC,MAC);}
if(response['share'].length>0){var cards=cards+this.templates.divider.format('Shared');for(var i=0;i<response['share'].length;i++){var MAC=response['share'][i]['mac'];var name=response['share'][i]['name'];var update=this.templates.update;if(parseInt(response['share'][i]['version'])==3){update="";}
cards=cards+this.templates.card.format(MAC,MAC,MAC,MAC,update,name,MAC,MAC,MAC,MAC,MAC,MAC,MAC,MAC,MAC);}}
...
getCards(){tools.showLoading("boards manager");tools.req('/manager/list',function(status,response){tools.hideDiag();if(status===200){var cards="";var boards=Object.keys(response['own']);this.total=boards.length;for(var i=0;i<boards.length;i++){var MAC=response['own'][i]['mac'];var name=response['own'][i]['name'];var shareWith=response['own'][i]['shareWith'];var cluster=(response['own'][i]['cluster']=="1")?'checked':'';cards=cards+this.templates.card.format(MAC,MAC,MAC,MAC,MAC,name,MAC,MAC,MAC,MAC,cluster,MAC,MAC,shareWith,MAC);}
if(response['share'].length>0){this.total=this.total+response['share'].length;var cards=cards+this.templates.divider.format('Shared');for(var i=0;i<response['share'].length;i++){var MAC=response['share'][i]['mac'];var name=response['share'][i]['name'];cards=cards+this.templates.shared.format(name,MAC,MAC);}}
...
if(email.value){console.log(email.value);console.log(tools.getCookie('Username'));if(tools.getCookie('Username')!=email.value){tools.show("popup2");tools.hide("popup3");tools.hide("delete");}else if(tools.getCookie('Username')==email.value){tools.hide("popup2");tools.hide("delete");if(pw.value){if(!filter_pw.test(pw.value)){tools.show("popup3");}else{tools.show("delete");tools.hide("popup3");return true;}}}}else{tools.hide("popup2");tools.hide("popup3");tools.hide("delete");}
return false;}}
