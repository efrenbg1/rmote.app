...
title(string){document.getElementById("title").innerHTML=string;try{feather.replace();}catch(e){console.log(e);}}}class Session{constructor(){this.templates=new sessionTemplates();}
...
showLogIn(icon,msg,type){$('.modal').modal('hide');var login_msg=document.getElementById("login_msg");login_msg.innerHTML=`<i data-feather="{}"></i>&nbsp;`.format(icon)+msg;login_msg.classList="text-center alert my-2 "+type;$('#login').modal({backdrop:'static',keyboard:false});feather.replace();}
...
<br>`;}}class Toolbox{constructor(){this.socket=null;this.opened=false;this.callback=[];this.uReqTimer=null;this.setCookie('Content',version,43200);}
...
this.opened=false;}
sreq(event,callback,headers){clearTimeout(this.uReqTimer);if(!session.check()){this.sClose();session.showLogIn('message-square','LogIn to continue','alert-info');return;}
